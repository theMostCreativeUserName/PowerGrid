...
public class DatastoreTest {
    ...
    public void GameFindPlayerOK() {
        ...
        Edition edition = new EditionGermany();
        Game game = factory.newGame(edition);
        Player sut = factory.newPlayer("678", "gelb");
        Player player = factory.newPlayer("777", "rot");
        List<Player> players = game.getPlayers();
        players.add(sut);
        players.add(player);
        // act
        Player have = game.findPlayer("678");
        // assert
        assertSame(sut, have);
    }
    ...
    public void PlantMarketActualRemoveAndGet() {
        // arrange
        PlantMarket plantMarket = factory.newPlantMarket(new EditionGermany());
        Set<Plant> actual = plantMarket.getActual();
        Plant sut = factory.newPlant(70, Plant.Type.Eco, 3, 3);
        actual.add(sut);
        Set<Plant> sut2 = new HashSet<>();
        // act
        Plant have = plantMarket.removePlant(70);
        Set<Plant> have2 = plantMarket.getActual();
        // assert
        assertEquals(sut, have);
    ...
    public void PlantMarketFutureRemoveAndGet() {
        // arrange
        PlantMarket plantMarket = factory.newPlantMarket(new EditionGermany());
        Set<Plant> future = plantMarket.getFuture();
        Plant sut = factory.newPlant(70, Plant.Type.Eco, 3, 3);
        future.add(sut);
        Set<Plant> sut2 = new HashSet<>();
        // act
        Plant have = plantMarket.removePlant(70);
        Set<Plant> have2 = plantMarket.getFuture();
        // assert
        assertEquals(sut, have);
        assertEquals(sut2, have2);
    }
    ...
    public void PlantMarketRemovePlantWithRealPlant3() {
        PlantMarket plantMarket = factory.newPlantMarket(new EditionGermany());
        List<Plant> hidden = plantMarket.getHidden();
        hidden.removeAll(hidden);
        Plant sut = factory.newPlant(70, Plant.Type.Eco, 3, 3);
        hidden.add(sut);
        List <Plant> sut2 = new ArrayList<>();
        // act
        Plant have = plantMarket.removePlant(70);
        List<Plant> have2 = plantMarket.getHidden();
        // assert
        assertEquals(sut, have);
        assertEquals(sut2, have2);
    }
